...
shexml_first_part = r"""
IMPORT <ShExMLTemplates/partial/LinksHeader.shexml>
SOURCE links_file <links/links_"""

shexml_second_part = r""".json>
IMPORT <ShExMLTemplates/partial/LinksIteratorsAndShapes.shexml>
"""
...
def convert_to_rdf(i, created_files, folder):
    ...
    output_filename = "./shexmlOutputLinks/links_" + str(index + 1) + ".ttl"
    content_filename = "./" + folder + "/links_" + str(index + 1) + ".json"
...
if __name__ == '__main__':
    ...
    print("Creating ShExML files for links in folder: " + folder)

    index = 0
    for i in os.scandir(folder):
        filename = "links_" + str(index + 1) + ".shexml"
        f = open("./shexmlRulesLinks/" + filename, "w", encoding="utf-8")
        f.write(shexml_first_part + str(index + 1) + shexml_second_part)
        f.close()
        created_files.append("./shexmlRulesLinks/" + filename)
