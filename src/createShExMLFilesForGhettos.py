...
def convert_to_rdf(i, created_files, folder):
    ...
    output_filename = "./shexmlOutputGhettos/ghettos_" + str(index + 1) + ".ttl"
    content_filename = "./" + folder + "/ghettos_" + str(index + 1) + ".json"
...
if __name__ == '__main__':
    ...
    for i in os.scandir(folder):
        filename = "ghettos_" + str(index + 1) + ".shexml"
        f = open("./shexmlRulesGhettos/" + filename, "w")
        f.write(shexml_first_part + str(index + 1) + shexml_second_part)
        f.close()
        created_files.append("./shexmlRulesGhettos/" + filename)
